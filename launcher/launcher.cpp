...
void PrintError(const char *funcName) {
  ...
  printf("mod_flatout2.exe: %s failed: %s\n", funcName, lpMsgBuf);
  LocalFree(lpMsgBuf);
  ExitProcess(9009);
}
...
int main(int argc, char *argv[]) {
  ...
  const char *exePath = "/media/data/Games/FlatOut2/PC_mod/FlatOut2.exe";
...

  {
    ...
    if (hdl == INVALID_HANDLE_VALUE) {
      ...
      if (hdl == INVALID_HANDLE_VALUE) {
        printf("mod_flatout2.exe: cannot open: %s\n", exePath);
        ExitProcess(9009);
      }
      ...
      if (hdl == INVALID_HANDLE_VALUE) {
        printf("mod_flatout2.exe: cannot create: %s\n", laaPath.c_str());
        ExitProcess(9009);
      }
  ...
  if (!DetourCreateProcessWithDllA(
          ...
          NULL,                                         // lpEnvironment,
          "/media/data/Games/FlatOut2/PC_mod/",         // lpCurrentDirectory,
          &startInfo,                                   //
          &processInfo,
          "/home/lukas/github/flatout/build/launcher/libfo2mod.dll",
