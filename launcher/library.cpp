...
struct CacheHandle {
  HANDLE handle;
  const bfcache::CacheFile *file = nullptr; // Yay let's segfault everything :)
...
WINBOOL WINAPI NewReadFile(HANDLE hFile, LPVOID lpBuffer,
  ...
  if (auto found = FindHandle(hFile); found != openedCacheHandles.end()) {
    const size_t toRead = std::min(found->size, size_t(found->virtualPosition +
                                                       nNumberOfBytesToRead)) -
                          found->virtualPosition;
    bfcache::GetFileData(*found->file, static_cast<char *>(lpBuffer),
...
static const std::set<std::string_view> BLOCKED_FILES{
    "filesystem",
    "patch",
    "data/video/nvidia.avi",
    "data/video/empire.avi",
    "data/video/bugbear.avi",
    "data/video/intrognl.avi",
    "data/video/introinl.avi",
};
...
HANDLE WINAPI NewFindFirstFileA(LPCSTR lpFileName,
  ...
  if (BLOCKED_FILES.contains(normFileName)) {
    return INVALID_HANDLE_VALUE;
  }

  if (normFileName.starts_with("data/")) {
    normFileName.erase(0, 5);
    ...
    try {
      auto currentCacheFile = bfcache::GetFile(normFileName);

      if (currentCacheFile) {
        *lpFindFileData = {};
        strcpy(lpFindFileData->cFileName,
               fileView.data() + fileView.find_last_of('\\') + 1);
        lpFindFileData->nFileSizeLow = bfcache::FileSize(*currentCacheFile);
  ...
  if (normFileName.starts_with("data/")) {
    ...
    if (currentCacheFile) {
      ...
      openedCacheHandles.emplace_back(CacheHandle{
          ...
          .size = bfcache::FileSize(*currentCacheFile),
      });
...
struct MappedMod {
  ...
  MappedMod(wchar_t *path)
      : cache(path), stream(std::make_unique<std::ifstream>(Changepath(path),
                                                            std::ios::binary)) {
    bfcache::AddCachePair(static_cast<bfcache::Cache *>(cache.data),
                          stream.get());
  }
};
...
void LoadCacheFolder() {
  ...
  GetFullPathName(L"mods/*.bfmod", MAX_PATH, absPath, NULL);
